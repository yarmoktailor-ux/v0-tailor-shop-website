...
import { 
  ...
  ShoppingBag // تم إضافة الاستيراد الناقص هنا
} from "lucide-react"
...
interface FabricsSectionProps {
  ...
  // تم إبقاء الخصائص الإضافية التي طلبتها
...
export function FabricsSection({
  ...
  // حل مشكلة توافق الخادم مع العميل (Hydration)
  ...
  // منع الرندرة قبل التحميل لتجنب أخطاء Next.js
  ...
  return (
    <section id="fabrics" className="py-12 px-4 max-w-6xl mx-auto">
      ...
      {/* رأس القسم مع ميزة الدخول السري لمحل اليرموك */}
        ...
        {fabrics.map((fabric) => {
          ...
          return (
            <div key={fabric.id} className={`group relative rounded-2xl border bg-card transition-all duration-300 ${order.qty > 0 ? "border-primary ring-1 ring-primary shadow-lg shadow-primary/10" : "border-border shadow-sm"}`}>
              ...
              {/* صورة القماش - مخصصة للأثواب الرجالية */}
              ...
              <div className="p-6">
                ...
                {order.qty > 0 && (
                  <div className="mt-6 pt-6 border-t border-dashed border-border space-y-5 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-muted-foreground uppercase font-bold px-1">ملاحظات خاصة</label>
                      <textarea 
                        placeholder="مثلاً: خياطة دبل، جيب مخفي..."
        ...
        {isAdmin && (
          showAdd ? (
            <div className="border-2 border-primary/30 bg-primary/5 rounded-2xl p-8 space-y-4 shadow-xl animate-in slide-in-from-bottom-4">
              <h3 className="font-bold text-xl text-primary flex items-center gap-2"><Plus size={24}/> إضافة ثوب جديد</h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                <input type="number" placeholder="السعر (ر.ي)" className="w-full p-3 border rounded-lg bg-background" value={newFabric.price} onChange={e => setNewFabric({...newFabric, price: e.target.value})} />
                <input type="number" placeholder="السعر القديم" className="w-full p-3 border rounded-lg bg-background" value={newFabric.oldPrice} onChange={e => setNewFabric({...newFabric, oldPrice: e.target.value})} />
              </div>
              <textarea placeholder="وصف القماش..." className="w-full p-3 border rounded-lg bg-background h-24" value={newFabric.description} onChange={e => setNewFabric({...newFabric, description: e.target.value})} />
              <div className="flex gap-3 pt-2">
                <button onClick={addFabric} className="flex-1 bg-primary text-white py-3 rounded-lg font-bold">إضافة الثوب</button>
